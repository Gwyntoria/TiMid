	...
	if len(push_url) > 0:
			...
			for line in fr:
				...
				if room_type == 1:
					if room_name_upper[0] == 'Y':
						...
						if 'multifilesrc' in line and '#' not in line:
							line = '        multifilesrc location=/home/pi/wawaji/music/tuibijioriginalbg.aac loop=true ! aacparse ! mux.\n'
						...
						if 'sharpness' in line and '#' not in line:
							line = '        keyframe-interval=25 sharpness=80 bitrate=3000000 saturation=5 exposure-compensation=0 do-timestamp=true rotation=90\n'
							update_supervisorctl = True
		...
		if not had:
			...
			if room_type == 1:
				if room_name_upper[0] == 'Y':
					update_supervisorctl = True
					music_file = 'tuibijioriginalbg.aac'
					...
					line = '[program:WaCamera]\ncommand=/usr/bin/gst-launch-1.0\n        -e flvmux name=mux ! rtmpsink location=\''+push_url+'\'\n        -v rpicamsrc preview=0 annotation-mode=custom-text+date+time+black-background annotation-text=\''+room_name_upper+' \' annotation-text-size=32'+timestamp_color+'        keyframe-interval=25 sharpness=80 bitrate=3000000 saturation=5 exposure-compensation=0 do-timestamp=true rotation=90\n        ! \'video/x-h264,width=720,height=1280,framerate=30/1,profile=high\' ! h264parse ! queue ! mux.\n        alsasrc device="hw:0,0" ! queue ! voaacenc bitrate=48000 ! aacparse ! queue ! mux.\n'+'stdout_logfile=/home/pi/wawaji/WaCamera.log\nredirect_stderr=true\nautostart=true\nautorestart=true\nstopasgroup=true\nkillasgroup=true\n'
